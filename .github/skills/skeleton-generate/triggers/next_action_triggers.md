# Skeleton Generate Next Action Triggers
...
|----|---------|-----------|--------|------|
| T1 | `output/{domain}_agent/CLAUDE.md` が作成された | `domain-skills-generate` | 1 | ドメイン固有Skills群を生成 |
| T2 | ユーザーが「個別Skillを追加/編集」と明示した | `skill-maintenance` | 2 | 個別メンテに切り替え |

## スキップ条件
以下の場合のみ、起動条件を満たしても実行をスキップできる:
- ユーザーが明示的に「骨格のみ」と指定した場合
- 検証目的のドライラン（反映なし）の場合
