    ...
    - id: "scenario_b"
      ...
      steps:
        - order: 1
          name: "ヒアリング準備"
          tasks:
            - "{{patterns.flow_private_date}}/agent_discovery_brief.md を作成し、課題・対象ユーザー・期待成果を整理する。"
        ...
        - order: 3
          ...
          tasks:
            ...
            - "トリガー名称・テンプレート構造・主要ステップ案を整理し、コメントは Flow/Private に残す。"
        - order: 4
          name: "ビルド準備完了判定"
          tasks:
  ...
  feedback_cycle:
    steps:
      - id: "evaluation_report"
        name: "評価レポート作成"
        tasks:
          - "Flow/Public に evaluation_report_{{meta.today}}.md を作成し、Syntax・Functional・Goal・Utility の観点で評価する。"
          - "判定根拠と改善点を出典・日時・担当者付きで記録する。"
      - id: "improvement_tasks"
        name: "改善タスク管理"
        tasks:
          - "{{patterns.backlog_root}} 等の管理票に改善タスクを登録し、完了条件と期限を明示する。"
          - "再発する課題は Flow/Private 側に原因分析メモを残す。"
      - id: "rule_update"
        name: "ルール更新と再配布"
        tasks:
...
quick_reference:
  phases:
    - order: 1
      name: "Phase 1: 設計準備"
      focus: "ドメイン分析・要件定義・機能設計を整理し、成功基準とKPIを合意する。"
      deliverables:
        - "TaskリストYAML"
        - "要件定義メモ"
    - order: 2
      name: "Phase 2: ルール実装"
      focus: "00_master_rules.mdc と {domain}_paths.mdc を基軸に01〜15番の機能ルールをテンプレ準拠で作成する。"
      deliverables:
        - ".cursor/rules/*.mdc"
    - order: 3
      name: "Phase 3: 品質保証"
...
available_actions:
  ...
  - name: "execute_shell"
    schema:
      ...
      parameters:
        command: "mkdir -p Stock/documents Flow/Public Flow/Private"
