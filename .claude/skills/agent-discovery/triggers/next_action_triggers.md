# Agent Discovery Next Action Triggers
...
|----|---------|-----------|--------|------|
| T1 | Flow配下に `draft_requirements.md` と `framework_research.md` が作成/更新された | `skeleton-generate` | 1 | エージェント骨格を生成 |

## スキップ条件
以下の場合のみ、起動条件を満たしても実行をスキップできる:
- ユーザーが明示的に「ヒアリングのみ」と指定した場合
- 検証目的のドライラン（反映なし）の場合
