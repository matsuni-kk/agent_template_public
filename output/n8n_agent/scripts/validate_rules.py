...
def check_master_triggers_format(path: Path, content: str) -> List[str]:
    """00_master_rules.mdc の master_triggers が3フィールド形式かをチェック"""
    errors: List[str] = []
    ...
    def parse_value(prefix: str, line: str) -> str:
    ...
    while i < len(lines):
        ...
        if re.match(r"^\s*-\s+trigger:\s*", line):
            ...
            i += 1
            while i < len(lines) and not re.match(r"^\s*-\s+trigger:\s*", lines[i]):
                cur = lines[i]
                ...
                if key_match:
                    ...
                    keys.add(key)
                    if key == "rule":
                        rule_value = parse_value("rule", cur)
                    if key == "description":
                        description_value = parse_value("description", cur)
                i += 1
...
def check_master_triggers_structure(path: Path, content: str) -> List[str]:
    ...
    def next_nonempty_index(from_idx: int) -> Optional[int]:
        j = from_idx
...
def check_quality_metrics(path: Path, content: str) -> List[str]:
    ...
    # 6. 質問数チェック（5-10個推奨）
    # NOTE:
    # - `prompt_why_questions:` のようなセクション名に部分一致して誤検知しないよう、
    #   ファイル全体の `- key:` 行をカウントする（質問セクションは `- key:` を前提とする）。
    question_items = re.findall(r'^\s*-\s+key:\s*', content, re.MULTILINE)
    ...
    # NOTE:
    # - Markdownテンプレートは先頭行が `# ...` になりやすく、`\n#` で切ると0文字扱いになる。
    # - `*_template: |` ブロックを複数抽出し、最長ブロックで判定する。
    template_blocks: List[str] = []
    for m in re.finditer(
        r'^[A-Za-z0-9_]+_template:\s*\|\s*\n(.*?)(?=^[A-Za-z0-9_]+:\s|\Z)',
        content,
        re.MULTILINE | re.DOTALL,
    ):
        template_blocks.append(m.group(1).strip())

    if template_blocks:
        longest_template = max(template_blocks, key=len)
        template_char_count = len(longest_template)
...
def validate() -> int:
    ...
    for file_path in files:
        ...
        file_errors.extend(check_master_triggers_format(rel, text))
        file_errors.extend(check_master_triggers_structure(rel, text))
