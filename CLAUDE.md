...
agent_creation_tasks:
  description: "ヒアリングで蓄積した要件と合意事項を基にエージェント作成工程を管理する進捗タスクリスト。ユーザーと共有し、必要に応じて項目を追加・削除する。初回の作成支援時のみ本テンプレートリポジトリの Flow 階層（例: Flow/agent_build/{{meta.year_month}}/{{meta.today}}_agent_creation_tasklist.md）に保存し、要件定義ファイルと逐次同期する。外部タスク機能を併用する場合でも、要件定義ファイルは必ず create_markdown_file で生成・更新することが必須。生成後のエージェントが独立運用する際は、このタスクリストは使用せず、各リポジトリ固有のフローで管理する。"
  default_items:
    - id: "configure_root_path"
      detail: "`templates.root_dir` と `root: \"{{templates.root_dir}}/{{agent_name}}\"` を実行環境に合わせて設定し、出力先パスを確定する。設定内容はタスクリストに記録する。"
    ...
    - id: "create_requirements_file"
      detail: "要件定義ファイル（例: {{patterns.flow_day_dir}}/draft_requirements.md）を create_markdown_file で必ず生成し、ユーザーヒアリングで得た要件を逐次追記する。外部タスク機能を利用していてもこのファイルは必須とする。タスクリストの各ステップは要件ファイルを参照しながら実行・評価する。生成後にエージェントが独立運用する際は、派生先リポジトリ内で要件ファイルを運用する。"
      ...
      steps:
        ...
        - order: 3
          ...
          tasks:
            ...
            - "要件定義ファイル（{{patterns.draft_requirements}} 等）を create_markdown_file で必ず生成し、ヒアリング結果と仮設計を記録する（タスク機能の有無に関わらず必須）。"
            ...
            - "{domain}_paths.mdc やテンプレート内コメント（例: templates.root_dir）を環境に合わせて更新し、利用者が迷わないよう明示する。"
            - "02番以降で扱う成果物ごとに、`{domain}_paths.mdc` の patterns セクションへ出力ファイルパターン（例: {{patterns.output_requirements_report}}）とファイル名規約を追加する。"
            - "99_rule_maintenance.mdc のログ/スケルトン/タスクリスト参照パターンを生成エージェントの Flow/Stock 構造に合わせて編集し、対応キーを {domain}_paths.mdc に追加する（Flow/agent_build の参照はここで除去する）。"
      ...
      steps:
        - order: 1
          ...
          tasks:
            ...
            - "要件定義ファイル（{{patterns.draft_requirements}} 等）を create_markdown_file で必ず生成し、以降のヒアリングで得た情報を逐次反映する（外部タスク機能利用時も必須）。"
        - order: 2
          name: "要件定義セッション"
          tasks:
            - "目的・KPI・成功条件を質問し記録する。"
  ...
  feedback_cycle:
    steps:
      - id: "evaluation_report"
        name: "評価レポート作成"
        tasks:
          - "{{patterns.flow_day_dir}}/evaluation_report_{{meta.today}}.md を作成し、Syntax・Functional・Goal・Utility の観点で評価する。要件定義ファイルと突き合わせて未達事項・差分を記録する。"
...
│       └── {program}/projects/{project}/documents/
│           ├── 1_{domain1}/  # ドメイン固有ディレクトリ
│           ├── 2_{domain2}/  # (業界標準に基づく)
│           └── ...
├── Archived/                  # アーカイブ
│   └── keep                  # ディレクトリ構造保持用
  ...
  phases:
    - order: 1
      ...
      deliverables:
        - "TaskリストYAML"
        - "要件定義メモ"
        - "Flow/agent_build/{{meta.year_month}}/{{meta.today}}_agent_creation_tasklist.md（root設定と承認事項を記録）"
        - "{{patterns.draft_requirements}}（ヒアリング内容と合意事項）"
    - order: 2
      name: "Phase 2: ルール実装"
      focus: "00_master_rules.mdc と {domain}_paths.mdc を基軸に01〜15番の機能ルールをテンプレ準拠で作成する。要件定義ファイルを参照しながら機能を落とし込み、02番以降の各ルールは成果物ファイルの格納パターン（{{patterns.output_*}} 等）を `{domain}_paths.mdc` に追加して Flow/Stock/Archived の保存先を明示する。"
      deliverables:
        - ".cursor/rules/*.mdc"
        - "{domain}_paths.mdc に追記した出力パターン一覧"
    - order: 3
      name: "Phase 3: 品質保証"
      focus: "実行テストと evaluation_report_YYYY-MM-DD.md で Syntax/Functional/Goal/Utility を採点し、要件定義ファイルと照合して不足・差分を特定する。"
      deliverables:
        - "評価レポート"
    - order: 4
      name: "Phase 4: 継続改善"
...
available_actions:
  ...
  - name: "execute_shell"
    schema:
      ...
      parameters:
        command: "mkdir -p Stock/documents Flow/_template/YYYYMMDD"
...
implementation_requirements:
  fundamentals:
    ...
    parameter_rules:
      ...
      - "content 内の改行は \n\n で明確に区切る。"
      - "`templates.root_dir` と `root: \"{{templates.root_dir}}/{{agent_name}}\"` の設定内容を Flow/agent_build タスクリストに記録し、全ルールが同一の出力先階層を参照するようにする。"
  ...
  implementation_sequence:
    steps:
      ...
      - order: 6
        title: "動作テスト"
        actions:
          - "実際のトリガーでワークフローを実行し挙動を検証する。"
    important_notes:
      - ".cursor/rules/ が実際に参照されるルール格納場所である。"
      - "97/98/99番台ルールはテンプレ汎用文言のため、エージェント固有のパス・質問・承認フローへ必ず置き換える。"
      - "{domain}_paths.mdc と 00_master_rules.mdc をテンプレ初期状態から必ず更新し、ディレクトリ構成・master_triggers・メッセージをエージェント仕様に合わせる。"
...
プロジェクトの完了報告として使用できます。お疲れ様でした。"
  ...
  manual_workflow:
    ...
    steps:
      ...
      - "パスファイル修正"
      - "動作テスト"
      - "evaluation_report_YYYY-MM-DD.md での厳格品質評価"
      - "ブラッシュアップ要否判定"
      - "必要に応じたブラッシュアップ実施"
      - "ルール保存とコミット"
    notes:
...
agent_quality_framework:
  evaluation_steps:
    step6:
      ...
      dimensions:
        syntax_compliance:
          ...
          criteria:
            - "YAML構文が正確か"
            - "ルール番号体系 (00-99) に従っているか"
            - ".cursor/rules/ 構造と命名規則に準拠しているか"
            - "{{patterns.xxx}} 形式のパス参照が正確か"
          checklist: |-
            syntax_compliance:
              - yaml_syntax_accuracy: [1-5点]
              - rule_numbering_compliance: [1-5点]
              - file_structure_compliance: [1-5点]
              - path_pattern_compliance: [1-5点]
        ...
        functional_completeness:
          ...
          checklist: |-
            functional_completeness:
              - standard_framework_alignment: [1-5点]
              - workflow_completeness: [1-5点]
              - function_integration: [1-5点]
              - practical_usability: [1-5点]
        ...
        goal_alignment:
          ...
          checklist: |-
            goal_alignment:
              - initial_requirement_match: [1-5点]
              - target_user_fit: [1-5点]
              - efficiency_improvement: [1-5点]
              - learning_curve: [1-5点]
        ...
        practical_utility:
          ...
          checklist: |-
            practical_utility:
              - question_quality: [1-5点]
              - template_quality: [1-5点]
              - error_handling: [1-5点]
              - extensibility: [1-5点]

            # 4項目平均4.0以上で合格
    step7:
      title: "厳格な品質評価とドキュメント化"
      process:
        - "evaluation_report_YYYY-MM-DD.md を作成する。"
        - "各評価項目を1-5点で採点し、3.5点未満は要改善とする。"
        - "改善内容を具体的にフィードバックへ記載する。"
        - "フィードバックに基づきブラッシュアップ要否を判定する。"
      report_template: |-
        # エージェント品質評価報告書

        **評価対象**: {AgentName}エージェント
        **評価日**: {{env.NOW:date:YYYY-MM-DD}}
        **評価者**: [評価者名]

        ## 1. 記述規則準拠性評価

        ### 1.1 YAML構文正確性 [X.X/5.0]
        **評価基準**: トリガー・ステップ・アクションのYAML記法が正確か
        **評価結果**: X.X点
        **詳細フィードバック**:
        - ✅ 良い点: [正確なYAML記述箇所]
        - ❌ 問題点: [構文エラー・不正な記述]
        - 🔧 改善指示: [具体的な修正方法・正しい記述例]

        ### 1.2 ルール番号体系準拠 [X.X/5.0]
        **評価基準**: 00-99の番号体系に従った適切なファイル配置
        **評価結果**: X.X点
        **詳細フィードバック**:
        - ✅ 良い点: [適切なファイル番号・配置]
        - ❌ 問題点: [番号体系違反・不適切な配置]
        ...
        ### 1.3 ファイル構造準拠 [X.X/5.0]
        **評価基準**: .cursor/rules/構造とファイル命名規則への準拠
        **評価結果**: X.X点
        **詳細フィードバック**:
        - ✅ 良い点: [適切なファイル構造・命名]
        - ❌ 問題点: [構造違反・命名規則違反]
        ...
        ### 1.4 パスパターン準拠 [X.X/5.0]
        **評価基準**: {{patterns.xxx}}形式の正確な使用とパス参照の整合性
        **評価結果**: X.X点
        **詳細フィードバック**:
        - ✅ 良い点: [正確なパスパターン使用]
        - ❌ 問題点: [パスパターンエラー・参照不整合]
        ...
        **詳細フィードバック**:
        ...
        ### 2.2 ワークフロー網羅度 [X.X/5.0]
        **評価基準**: ドメインの主要業務プロセスが適切にカバーされているか
        **評価結果**: X.X点
        **詳細フィードバック**:
        - ✅ 良い点: [カバーされている業務プロセス]
        - ❌ 問題点: [不足している業務プロセス]
        ...
        ### 2.3 機能間連携 [X.X/5.0]
        **評価基準**: 各機能が相互に連携し、一貫したワークフローを形成しているか
        **評価結果**: X.X点
        **詳細フィードバック**:
        - ✅ 良い点: [連携が良好な機能群]
        - ❌ 問題点: [連携不備・矛盾点]
        ...
        ### 2.4 実務適用性 [X.X/5.0]
        **評価基準**: 実際の業務シーンで即座に使用可能な質問・テンプレートか
        **評価結果**: X.X点
        **詳細フィードバック**:
        - ✅ 良い点: [実用的な質問・テンプレート]
        - ❌ 問題点: [不十分・不適切な質問・テンプレート]
        - 🔧 改善指示: [具体的な修正方法]
        ...
        **ゴール適合性総合評価**: [平均点]/5.0
        ...
        ### 4.1 質問品質 [X.X/5.0]
        ### 4.2 テンプレート品質 [X.X/5.0]
        ### 4.3 エラーハンドリング [X.X/5.0]
        ### 4.4 拡張性 [X.X/5.0]
    ...
    step8:
      title: "戦略的ブラッシュアップ実施"
      decision_flow: |-
        brushup_decision_flow:
          step1: "評価MDファイル詳細レビュー"
          step2: "問題の優先度・影響度分析"
          step3: "改善コスト・効果分析"
          step4: "ブラッシュアップ要否の戦略判断"
          step5: "実施判定なら詳細計画策定"
          step6: "ブラッシュアップ実行"
          step7: "再評価・完了判定"
      criteria:
      ...
      action_levels:
        level1:
          description: "軽微修正 (1-2日)"
          actions:
            - "文言調整・表記統一"
            - "軽微なテンプレート修正"
            - "トリガー語彙の追加"
        level2:
          description: "機能改善 (3-5日)"
          actions:
            - "質問項目の再構成"
            - "テンプレート構造の見直し"
            - "パスパターンの最適化"
        level3:
          description: "構造変更 (1-2週間)"
          actions:
            - "機能統合・分離・再設計"
            - "ワークフロー全体見直し"
            - "新機能追加・不要機能削除"
        level4:
          description: "根本的見直し (2-4週間)"
          actions:
            - "ドメイン分析のやり直し"
            - "アーキテクチャの抜本的変更"
            - "エージェント全体の再設計"
